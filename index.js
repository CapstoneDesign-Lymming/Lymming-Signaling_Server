...
io.on("connection", function (socket) {
  ...
  socket.on("join", function (data) {
    if (!(data === null || data === void 0 ? void 0 : data.room)) return;
  ...
  socket.on("screenSharing", function (data) {
    ...
    console.log(
      "\uBC29 "
        .concat(room, "\uC5D0\uC11C \uD654\uBA74\uACF5\uC720\uC0C1\uD0DC: ")
        .concat(isScreenSharing)
    );
    socket
      .to(room)
      .emit("screenSharing", {
  ...
  socket.on("toggleMic", function (data) {
    console.log(
      "User ".concat(data.userId, " toggled mic: ").concat(data.isMicOn)
    );
  ...
  socket.on("toggleVideo", function (data) {
    console.log(
      "User ".concat(data.userId, " toggled mic: ").concat(data.isVideoOn)
    );
